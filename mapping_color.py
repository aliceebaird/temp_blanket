...
location = 'London' # match request output
...
df['color_code'], df['color_name'] = zip(*df[temp_metric].apply(
    lambda x: map_temp_to_color_hist_with_name(x, hist_bins, list(list_color_keys), colors)))
...
fig, ax = plt.subplots(figsize=(len(df), 20))  # Width is proportional to the number of days
